...
- 每个 secret 应包含完整的 cURL 命令，但不需要包含 `-c` 或 `--cookie-jar` 参数（系统会自动处理）
...
2. 对于每个配置的站点，并行执行对应的 cURL 命令
3. 系统会自动添加 `-c` 参数来保存响应中的 Set-Cookie 头到对应站点的 cookie 文件中
4. 下次执行时可以使用 `-b` 参数加载这些 cookie
